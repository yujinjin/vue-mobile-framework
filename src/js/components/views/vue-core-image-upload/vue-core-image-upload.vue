<template>
	<div class="g-core-image-upload-btn">
		...
		<div class="g-core-image-corp-container" v-bind:id="'vciu-modal-' + formID" v-show="hasImage" @click.stop.prevent @tap.stop.prevent>
			<crop ref="cropBox" :is-resize="resize && !crop" :ratio="cropRatio" :is-rotate="rotate"></crop>
			<div class="info-aside">
				<p class="btn-groups rotate-groups" style="float:left;">
					<button type="button" v-on:click.stop.prevent="doRotate" class="btn btn-rotate">↻</button>
					<button type="button" v-on:click.stop.prevent="doReverseRotate" class="btn btn-reverserotate">↺</button>
				</p>
				<p class="btn-groups" v-if="crop">
					<button type="button" v-on:click.stop.prevent="doCrop" class="btn btn-upload">{{cropBtn.ok}}</button>
					<button type="button" v-on:click.stop.prevent="cancel" class="btn btn-cancel">{{cropBtn.cancel}}</button>
				</p>
				<p class="btn-groups" v-if="resize && !crop">
					<button type="button" v-on:click.stop.prevent="doResize" class="btn btn-upload">{{ResizeBtn.ok}}</button>
					<button type="button" v-on:click.stop.prevent="cancel" class="btn btn-cancel">{{ResizeBtn.cancel}}</button>
				</p>
			</div>
		</div>
	</div>
